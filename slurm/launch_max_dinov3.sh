#!/bin/bash
# "Max push" sweep with DINOv3-base encoder + SAM2.1 Large mask decoder.
...
# at native resolution; only the encoder differs (sam2_enc_large -> dinov3_base).
#
# Usage:
#   bash slurm/launch_max_dinov3.sh
#
set -euo pipefail
cd /home/nfs/tals/galash

TAG="$(date +%Y%m%d_%H%M)"
# --no_amp: dinov3 + heavy aug + cnn_skip + finetune_decoder explodes BCE in
# epoch 1 under autocast (logits → Inf → sigmoid > 1 → BCE assert). Disabling
# AMP costs ~30% throughput but is the safest gradient-stability fix on B200.
...
for row in "${ROWS[@]}"; do
    read -r DS CFG <<< "$row"
    NAME="maxdinov3_${DS}_${TAG}"
    echo "--- $NAME  (dgx, encoder=dinov3_base  decoder=sam2_large  config=$CFG)"
    JID=$(env \
        ...
        sbatch \
            ...
            --mem=128G \
            --time=48:00:00 \
            --job-name="g-${DS}-dinov3" \
            --parsable \
            slurm/train.sbatch)
    echo "    job $JID  -> logs/slurm/train_${JID}_g-${DS}-dinov3.out"
    submitted+=("$JID")
done
